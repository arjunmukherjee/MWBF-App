    ...
    if (self.numberOfMessages > 0 )
    {
        ...
        if (user.activityNotifications && user.friendsAndChallengesNotifications)
        {
            if (newFriendCount > 0 && newChallengeCount > 0 && newMessageCount > 0)
                messageBody = [NSString stringWithFormat:@"You have %d new %@,3 %d new or removed %@ and %d new %@.",newFriendCount,friendStr,newChallengeCount,challengeStr,newMessageCount,notificationStr];
