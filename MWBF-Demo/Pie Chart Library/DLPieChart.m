...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        self.labelFont = [UIFont fontWithName:@"Trebuchet MS" size:12];
...
- (void)renderInLayer:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray labelArray:(NSArray*)labelArray
{
    if(!self.DLDataArray)
    ...
    */
    ...
    [layerHostingView setLabelFont:[UIFont fontWithName:@"Trebuchet MS" size:12]];
    ...
    [self drawLegends:layerHostingView dataArray:dataArray labelArray:labelArray];
}
...
-(void)drawLegends:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray labelArray:(NSArray*)labelArray
{
    UIView *legendsView = [[UIView alloc] init];
    legendsView.frame = CGRectMake(layerHostingView.pieCenter.x+layerHostingView.pieRadius+OFFSET*2, 10, 150, 300);
    [legendsView setBackgroundColor:[UIColor clearColor]];
    
    UIScrollView *legendsScrollView = [[UIScrollView alloc] init];
    legendsScrollView.frame = CGRectMake(0, 0, legendsView.frame.size.width, legendsView.frame.size.height+80);
    [legendsScrollView setBackgroundColor:[UIColor clearColor]];
    [legendsView addSubview:legendsScrollView];
    
    //legend view border
    //[legendsView.layer setBorderColor:[UIColor grayColor].CGColor];
    //[legendsView.layer setBorderWidth:1.0];
    ...
    legendX = layerHostingView.pieCenter.x+layerHostingView.pieRadius+OFFSET*2-30;
    legendY = OFFSET*2;
    legendWidth = layerHostingView.frame.size.width - OFFSET*4 - (layerHostingView.pieRadius*2) + 45;
    legendHeight = layerHostingView.pieRadius * 2 - OFFSET + 20;
    
    legendsView.frame = CGRectMake(legendX, legendY-50, legendWidth, legendHeight);
    legendsScrollView.frame = CGRectMake(0, 0, legendsView.frame.size.width, legendsView.frame.size.height+150);
    ...
    for (int i=0;i<dataArray.count;i++)
    {
        ...
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(35, y, legendWidth-OFFSET*2+5, 25)];
        lbl.text = [NSString stringWithFormat:@"%@",[labelArray objectAtIndex:i]];
